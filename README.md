...
- **Plain-English Summary:** a paragraph written for non-technical readers
- **Original Source Code:** the full source appended for reference

**Built for understanding and accountability.** Because the output calls out
assumptions, risks, and dependencies explicitly, non-technical stakeholders
can read, understand, and sign off on what code does. That helps with
compliance documentation, audit trails, code reviews involving
non-developers, and any case where someone has to defend a technical
decision to a non-technical audience.
...
- **Notebook support:** Jupyter (`.ipynb`), Quarto (`.qmd`),
  R Markdown (`.rmd`), with a cell-by-cell walkthrough
- **Markdown and plain-text output:** renders in GitHub, Obsidian, VS Code,
...
- **Desktop GUI:** no terminal required; drag and drop files
...
For each file you decode, you get two files in your Downloads folder:
...
|---|---|---|
| **Claude** (Anthropic) | No | `sk-ant-api03-…` |
| **ChatGPT** (OpenAI) | No | `sk-proj-…` |
| **Gemini** (Google) | Yes | `AIza…` |
| **Groq** | Yes, generous | `gsk_…` |
| **Mistral** | No | long random string |
...
- **API keys are stored in plaintext** at `~/.decode_for_humans/config.json` on your local machine. This file is excluded from version control by `.gitignore`. Do not share it or commit it to a repository.
